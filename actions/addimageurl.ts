...
export const addImageUrl = async (imageoData: z.infer<typeof AudioVideoImageUrlSchema>) => {
    ...
    try{
        ...
        const validatedFields = AudioVideoImageUrlSchema.safeParse(imageoData);
        if(validatedFields.error) return {failed: validatedFields.error.errors};

        const NewImageUrl = new Image({
            name: imageoData.name,
            url: imageoData.url,
