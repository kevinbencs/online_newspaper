...
export const addVideoUrl = async (videoData: z.infer<typeof AudioVideoImageDeleteUrlSchema>) => {
    ...
    try{
        ...
        const validatedFields = AudioVideoImageDeleteUrlSchema.safeParse(videoData);
        if(validatedFields.error) return {failed: validatedFields.error.errors};

        await Video.findOneAndDelete({url: videoData.url})
        await closeConnection();
