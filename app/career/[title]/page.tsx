...
import EditRedirect from "@/app/_components/career/editRedirect";
import  ChooseTypeOfTextItem from "@/app/_components/career/showCareerSSR";
...
const Page = async ({params}:{params:{title: string}}) => {
  ...
  return (
    <div className="min-h-screen pb-10">
      ...
      <EditRedirect url={decodeURIComponent(params.title.replaceAll('_', ' '))}/>
      <div>
        {success && <>{success.text.split('$').map((item: string) => <ChooseTypeOfTextItem s={item} key={uuid()}/>)}</>}
      </div>
    </div>
  )
}
