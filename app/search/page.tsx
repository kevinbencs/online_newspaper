...
const Page = async ({ searchParams }: {
  ...
  const res = await searchNews(searchParams.category, searchParams.text, searchParams.date_from, searchParams.date_to, searchParams.author, searchParams.page, searchParams.filter)
    ...
    <div className="relative">
      ...
      <div className="lg:flex mt-10 mb-10 lg:gap-32 lg:flex-wrap">
        <div className="lg:w-[calc(100%-450px)] text-center">
          <div className="mb-10">
            {res.success?.data.data && res.success.data.data.map(item => <Latest_important date={item.date} detail={item.detail} author={item.author} category={item.category} imageId={item.cover_img_id} title={item.title} key={item.id}
              link={`/${item.category}/${item.date.slice(0, 4)}/${item.date.slice(6, 8)}/${item.date.slice(10, 12)}/${item.title.replaceAll(' ', '-')}`} />)}
          </div>
