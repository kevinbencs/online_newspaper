...
export async function GET(req: NextRequest) {
    ...
    if (data.user?.app_metadata.twofa === 'true' && TwoFA) {
        ...
        if (!tokenRes) return NextResponse.json({ role: '', name: '', number: 0 }, { status: 200 });

        if (!process.env.TwoFA_URI) return NextResponse.json({ role: '', name: '', number: 0 }, { status: 200 });

        const decoded = await jwt.verify(TwoFA.value, process.env.TwoFA_URI!) as Decoded;

        if (decoded.id !== data.user.id) return NextResponse.json({ role: '', name: '', number: 0 }, { status: 200 });
