  ...
  if (res.success) return (
    <div className="relative">
      ...
      <div className="lg:flex mt-10 mb-10 lg:gap-32 lg:flex-wrap">
        <div className="lg:w-[calc(100%-450px)] text-center">
          <div className="mb-10">
            {res.success.data.map(item => <Category_menu_articles link={'/'+params.name.toLowerCase()+'/'+item.date.slice(0,4)+'/'+item.date.slice(6,8)+'/'+item.date.slice(10,12)+'/'+item.title.replaceAll(' ','-')} 
            date={item.date.replaceAll(' ','')} detail={item.detail} category_name={item.author} category_name_link={`/authors/${item.author.replaceAll(' ', '-')}`} imageId={item.cover_img_id} title={item.title} key={item.id} />)}
          </div>
          {lastPage.success && <Pagination searchParams={searchParams} lastPage={lastPage.success} params={params} />}
        </div>
        <div className="lg:w-80">
