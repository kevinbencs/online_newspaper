  ...
  useEffect(() => {
    ...
    .then(res => {
      if(res.data) {
        ...
        setParagraphInput(res.data.text.split('$').join('\n\n'));
        setParagraphPaywallInput(res.data.paywall_text.split('$').join('\n\n'));
        setFirstElementUrl(res.data.first_element_url);
        setFirstElementInput(res.data.first_element);
        setSidebar(res.data.sidebar ? 'Sidebar: yes': 'Sidebar: no');
        setCategoryInput(res.data.category)
        setCoverImageId(res.data.cover_img_id)
        setThemes(res.data.keyword)
        if(TextEnterRef.current) TextEnterRef.current.innerText = `${res.data.text.split('$').join('\n')}`
        setParagPlaceholder('');
        if(res.data.paywall_text !== '') setParagPaywallPlaceholder('');
        setImportantInput(res.data.important);
        setCategoryInput(res.data.category);
