...
const OptgroupSearch = (props: {optElement:{id:string, text:string}[], optInput: string, setOptInput: Dispatch<SetStateAction<string>>, placeHolder: string}) => {
    ...
    return (
        <label className='lg:w-[30%] w-full relative block'>
            <input ref={optRef} type="text" name='search_audio' onFocus={() => setOptClass('h-36')} onBlur={() => setOptClass('h-0')} className='focus-within:outline-none dark:text-white input-bordered border-b-2 block w-full bg-transparent pl-2' placeholder={props.placeHolder} value={props.optInput} onChange={handleChange} />
