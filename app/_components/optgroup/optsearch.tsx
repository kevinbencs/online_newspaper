...
const OptgroupSearch = (props: {optElement:{id:string, text:string}[], optInput: string, setOptInput: Dispatch<SetStateAction<string>>, placeHolder: string}) => {
    ...
    return (
        <label className='lg:w-[30%] w-full  block'>
            <input ref={optRef} type="text" name='search_audio' onFocus={() => setOptClass('h-36')} onBlur={() => setOptClass('h-0')} className='focus-within:outline-none border-b-2 block w-full bg-transparent pl-2' placeholder={props.placeHolder} value={props.optInput} onChange={handleChange} />
            <ul className={`${optClass} overflow-y-scroll sidebar absolute w-[100%] lg:w-30% dark:bg-neutral bg-base-200 duration-100 pl-2`}>
