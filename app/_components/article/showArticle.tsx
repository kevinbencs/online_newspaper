...
const jsxInText = (s: string) => {
  ...
  while (index2 > -1) {
      ...
      if (s.indexOf('<Link', index1) === index2 && index2 > -1) {
          ...
          result = createLink(s.slice(index2, s.indexOf('</Link>', index2) + 7), index2);
          ...
          textArray.push(s.slice(index1, index2));
          result = createAnchor(s.slice(index2, s.indexOf('</anchor_link>', index2) + 14), index2);
          if (typeof (result) !== 'number') {
              textArray.push(result);
          ...
          textArray.push(s.slice(index1, index2));
          result = createStrong(s.slice(index2, s.indexOf('</bold>', index2) + 7), index2);
          if (typeof (result) !== 'number') {
              textArray.push(result);
          ...
          textArray.push(s.slice(index1, index2));
          result = createEm(s.slice(index2, s.indexOf('</italic', index2) + 9), index2);
          if (typeof (result) !== 'number') {
              textArray.push(result);
...
const createLink = (s: string, index2: number) => {
  ...
  while (index3 > -1) {
      if (text.indexOf('<italic') === index3 && index3 > -1) {
          ...
          result = createEm(text.slice(index3, text.indexOf('</italic>', index3) + 9), index2);
          ...
          textArray.push(text.slice(index1, index3));
          result = createStrong(text.slice(index3, text.indexOf('</bold>', index3) + 7), index2);
...
const createAnchor = (s: string, index2: number) => {
  const indexHref: number = s.indexOf('(');
  const indexHrefEnd: number = s.indexOf(')');
...
const createStrong = (s: string, index2: number) => {
  ...
  while (index3 > -1) {
      if (text.indexOf('<italic') === index3 && index3 > -1) {
          ...
          textArray.push(text.slice(index1, index3));
          result = createEmText(text.slice(index3, text.indexOf('</italic>', index3) + 9), index2);
          if (typeof (result) !== 'number') {
              textArray.push(result);
              index1 = text.indexOf('</italic>', index3) + 9;
              index3 = text.indexOf('<', index1);
...
const createStrongText = (s: string, index2: number) => {
  const indexHrefEnd = s.indexOf('>');
  const indexTextEnd = s.indexOf('</bold', 1);
  const index = s.indexOf('<', 1);
...
const createEm = (s: string, index2: number) => {
  const indexHrefEnd = s.indexOf('>');
  ...
  while (index3 > -1) {
      if (text.indexOf('<bold') === index3 && index3 > -1) {
          textArray.push(text.slice(index1, index3));
          result = createStrongText(text.slice(index3, text.indexOf('</bold>', index3) + 7), index2);
...
const createEmText = (s: string, index2: number) => {
  const indexHrefEnd = s.indexOf('>');
  const indexTextEnd = s.indexOf('</italic');
  const index = s.indexOf('<', 1);
