...
const Miniaudio = (props: {
    ...
    return (
        <div className='fixed sm:bottom-4 bottom-0 sm:left-4 bg-slate-500 rounded-lg p-2 w-full left-0 sm:w-[230px]  text-white z-[11]'>
            ...
            <input type="range" name='time2' min={0} max={props.audioTime} value={props.currentTime}
                onChange={props.handleRangeChange} className='h-1 w-[100%] block mb-4'
            />
            <div className='flex items-center gap-2'>
                <label className="swap">
                    <input type="checkbox" checked={props.muted} onChange={props.volumeHandleClick} />
                ...
                <input type="range" name="volume2" min={0} max={1} step={0.02} className='h-1' value={props.volume} onChange={(e) => props.setVolume(Number(e.target.value))} />
            </div>
        </div>
    )
}
