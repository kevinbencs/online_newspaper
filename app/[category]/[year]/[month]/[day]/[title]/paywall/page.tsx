...
import { getArtUrl, getPaywallArtUrl } from '@/actions/getArtUrl';
...
export async function generateStaticParams() {
  const res = await getPaywallArtUrl();
...
const Page = async ({ params, searchParams }: { params: { category: string, year: string, month: string, day: string, title: string }, searchParams: { source: string } }) => {
  ...
  const date = params.year + '. ' + params.month + '. ' + params.day + '.'
  const res = await getPaywallArticle({ Article: decodeURIComponent(params.title.replaceAll('_', ' ').replace('nb20', '?')), date, source: searchParams.source });
